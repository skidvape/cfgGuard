...
That version was around 3000 lines long, and supported Lua versions 5.1, 5.2, 5.3, and 5.4.
...
doesn't overcomplicate it with support of unreachable cases.

After quite a bit of work and benchmarking, this is what I was left with.
...
local AND	= bit32.band                -- 2 arguments
local OR	= bit32.bor                 -- 2 arguments
local XOR  = bit32.bxor                -- 2..5 arguments
local SHL  = bit32.lshift              -- second argument is integer 0..31
local SHR  = bit32.rshift              -- second argument is integer 0..31
local ROL  = bit32.lrotate    -- second argument is integer 0..31
local ROR  = bit32.rrotate    -- second argument is integer 0..31

local XOR_BYTE = XOR               -- XOR of two bytes (0..255)
...
local function sha1_feed_64(H, str, offs, size)
  ...
  for pos = offs, offs + size - 1, 64 do
     ...
     for j = 1, 20 do
        local z = ROL(a, 5) + AND(b, c) + AND(-1-b, d) + 0x5A827999 + W[j] + e        -- constant = math.floor(TWO_POW_30 * sqrt(2))
        e = d
        d = c
        c = ROR(b, 2)
        b = a
     ...
     for j = 21, 40 do
        local z = ROL(a, 5) + XOR(b, c, d) + 0x6ED9EBA1 + W[j] + e                    -- TWO_POW_30 * sqrt(3)
        e = d
        d = c
        c = ROR(b, 2)
        b = a
     ...
     for j = 41, 60 do
        local z = ROL(a, 5) + AND(d, c) + AND(b, XOR(d, c)) + 0x8F1BBCDC + W[j] + e   -- TWO_POW_30 * sqrt(5)
        e = d
        d = c
        c = ROR(b, 2)
        b = a
     ...
     for j = 61, 80 do
        local z = ROL(a, 5) + XOR(b, c, d) + 0xCA62C1D6 + W[j] + e                    -- TWO_POW_30 * sqrt(10)
        e = d
        d = c
        c = ROR(b, 2)
        b = a
...
local function keccak_feed(lanes_lo, lanes_hi, str, offs, size, block_size_in_bytes)
  ...
  for pos = offs, offs + size - 1, block_size_in_bytes do
     ...
     for round_idx = 1, 24 do
        ...
        local D_lo = XOR(C1_lo, C3_lo * 2 + (C3_hi % TWO_POW_32 - C3_hi % TWO_POW_31) / TWO_POW_31)
        local D_hi = XOR(C1_hi, C3_hi * 2 + (C3_lo % TWO_POW_32 - C3_lo % TWO_POW_31) / TWO_POW_31)
        ...
        D_lo = XOR(C2_lo, C4_lo * 2 + (C4_hi % TWO_POW_32 - C4_hi % TWO_POW_31) / TWO_POW_31)
        D_hi = XOR(C2_hi, C4_hi * 2 + (C4_lo % TWO_POW_32 - C4_lo % TWO_POW_31) / TWO_POW_31)
        ...
        D_lo = XOR(C3_lo, C5_lo * 2 + (C5_hi % TWO_POW_32 - C5_hi % TWO_POW_31) / TWO_POW_31)
        D_hi = XOR(C3_hi, C5_hi * 2 + (C5_lo % TWO_POW_32 - C5_lo % TWO_POW_31) / TWO_POW_31)
        ...
        D_lo = XOR(C4_lo, C1_lo * 2 + (C1_hi % TWO_POW_32 - C1_hi % TWO_POW_31) / TWO_POW_31)
        D_hi = XOR(C4_hi, C1_hi * 2 + (C1_lo % TWO_POW_32 - C1_lo % TWO_POW_31) / TWO_POW_31)
        ...
        D_lo = XOR(C5_lo, C2_lo * 2 + (C2_hi % TWO_POW_32 - C2_hi % TWO_POW_31) / TWO_POW_31)
        D_hi = XOR(C5_hi, C2_hi * 2 + (C2_lo % TWO_POW_32 - C2_lo % TWO_POW_31) / TWO_POW_31)
   ...
   repeat
      ...
      repeat
         ...
         if d*d > p then -- next prime number is found
            ...
            R = mul(table.create(1, R - R % 1), one, 1.0, 2)
...
for width = 224, 256, 32 do
   ...
   if XOR64A5 then
      ...
      H_hi = {}
      for j = 1, 8 do
         H_lo[j] = XOR(sha2_H_lo[j], 0xA5A5A5A5) % 4294967296
		 H_hi[j] = XOR(sha2_H_hi[j], 0xA5A5A5A5) % 4294967296
      end
   end
...
local function sha256ext(width, message)
   ...
   local H, length, tail = table.pack(table.unpack(sha2_H_ext256[width])), 0.0, ""
   ...
   local function partial(message_part)
      if message_part then
		local partLength = #message_part
         ...
         if tail then
            local final_blocks = table.create(3)--{tail, "\128", string.rep("\0", (-9 - length) % 64 + 1)}
				final_blocks[1] = tail
				final_blocks[2] = "\128"
            ...
            for j = 4, 10 do
               length = length % 1 * 256
               final_blocks[j] = string.char(length - length % 1)
            end
...
local function md5(message)
   ...
   local H, length, tail = table.create(4), 0.0, ""
	H[1], H[2], H[3], H[4] = table.unpack(md5_sha1_H, 1, 4)
   ...
   local function partial(message_part)
	...
	local partLength = #message_part
         ...
         if tail then
            ...
            local lanes_used = 0
            local total_lanes = block_size_in_bytes / 8
			total_lanes = total_lanes - total_lanes % 1
            local qwords = {}
            ...
            local function get_next_qwords_of_digest(qwords_qty)
               ...
               qwords_qty = math.min(qwords_qty, total_lanes - lanes_used)
				qwords_qty = qwords_qty - qwords_qty % 1
               ...
               return
                  string.gsub(table.concat(qwords, "", 1, qwords_qty), "(..)(..)(..)(..)(..)(..)(..)(..)", "%8%7%6%5%4%3%2%1"),
                  qwords_qty * 8
            end
...
local function pad_and_xor(str, result_length, byte_for_xor)
	return string.gsub(str, ".", function(c)
		return string.char(XOR_BYTE(string.byte(c), byte_for_xor))
